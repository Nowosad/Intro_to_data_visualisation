...
- R has several systems for making graphics - **base R**, the **lattice** package, and the **ggplot2** package are the most popular ones
...
p <- ggplot(data=gapminder2007, aes(x=lifeExp, fill=continent)) +
        geom_histogram() + xlab('Life expectancy') + ylab(NULL) 
p <- p + ggtitle('Histogram') + labs(fill = "Continent: ")
p
```
...
p <- ggplot(data=gapminder2007, aes(x=lifeExp, fill=continent)) +
        geom_histogram() + xlab('Life expectancy') + ylab(NULL) 
p <- p + ggtitle('Histogram') + labs(fill = "Continent: ") + theme_bw()
```
